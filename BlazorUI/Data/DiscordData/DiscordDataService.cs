...
using Sparta.BlazorUI.Helpers;

namespace Sparta.BlazorUI.Data.DiscordData;
...
public class DiscordDataService
{
    ...
    {
        return _context.DC_Embeds.ToList();
    }

    public List<DiscordGuild> GetGuilds()
    {
        return _context.DC_Guilds.ToList();
    }

    public List<DiscordAuthor> GetAuthors()
    {
        return _context.DC_Authors.ToList();
    }

    public List<DiscordModal> GetModals()
    {
        return _context.DC_Modals.ToList();
    }

    public List<DiscordMessageComponent> GetMessageComponents()
    {
        return _context.DC_MessageComponents.ToList();
    }

    public DiscordMessageComponent? GetComponent(string id)
    {
        return _context.DC_MessageComponents.Find(id);
    }
    ...
    public List<ComponentDataModel> GetComponentModels()
    {
        ...
        foreach (var dbComponet in dbComponets)
            components.Add(new ComponentDataModel
            {
                ...
                EmbedTitle = string.Join(", ",
                    _context.DC_Embeds.Where(y => y.MessageComponents.Any(z => z.Id == dbComponet.Id))
                        .Select(x => x.Title).ToList()).Truncate(20),
    ...
    public DiscordEmbed? GetEmbed(long id)
    {
        return _context.DC_Embeds.FirstOrDefault(x => x.Id == id);
    }
    ...
    public async Task SaveEmbedAsync(DiscordEmbed embed, long channelId, long selectedAuthorId)
    {
        ...
        else
        {
            ...
            if (authors.Any()) await authors.ForEachAsync(x => x.Embeds.Remove(embed));
        }
