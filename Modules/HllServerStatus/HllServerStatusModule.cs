...
namespace Sparta.Modules.HllServerStatus
{
    public class HllServerStatusModule(DiscordAccess discord, SpartaDbContext context, RconDataAccess rcon, BattleMetricsDataAccess bm) : IModule
    {
        ...
        private Embed GenerateEmbed(MdModule module)
        {
            ...
            var info = rcon.GetServerInfo(module.Server.Url, module.Server.Port, module.Server.Username, module.Server.Password);
            var teamView = rcon.GetTeamView(module.Server.Url, module.Server.Port, module.Server.Username, module.Server.Password);
            var mods = rcon.GetIngameMods(module.Server.Url, module.Server.Port, module.Server.Username, module.Server.Password).Select(m => long.Parse(m.SteamId ?? "0")).ToList();
            var battleMetricsRank = bm.GetBattlemetricsRank(ConfigHelper.GetConfig("Battlemetrics", "Url") ?? "",
                long.Parse(module.MdParameters.First(p => p.Name == nameof(HllServerStatusParameters.BattleMetricsId)).Value));
            ...
            var winningTeam = winning == "allied" ? teamView.Allies : teamView.Axis;
            var losingTeam = losing == "allied" ? teamView.Allies : teamView.Axis;
            ...
            return new EmbedBuilder()
                ...
                .AddField("**Verteilung**", $"{StaticResources.Graphics[winning]} {info.Players[winning]} - {info.Players[losing]} {StaticResources.Graphics[losing]}", true)
                .AddField("**Karte**", $"{info.CurrentMap?.Name}", true)
                .AddField("**Ø-Level**", $"{StaticResources.Graphics[winning]} {winningTeam?.GetAverageLevel()} - {losingTeam?.GetAverageLevel()} {StaticResources.Graphics[losing]}", true)
                .AddField("**Zwischenstand**", $"{StaticResources.Graphics[winning]} {info.Score[winning]} - {info.Score[losing]} {StaticResources.Graphics[losing]}", true)
                .AddField("**Nächste Karte**", $"{info.NextMap?.Name}", true)
                .AddField("**Battlemetrics Rang**", battleMetricsRank, true)
                .AddField("**VIPs**", $"{StaticResources.Graphics[winning]} {winningTeam?.GetVips()} - {losingTeam?.GetVips()} {StaticResources.Graphics[losing]}", true)
                .AddField("**Spielzeit**", $"{info.CurrentMap?.StartTime.Subtract(DateTime.Now):h\\:mm} Stunden", true)
                .AddField("**Taurus Spieler**", winningTeam?.GetTaurusPlayers() ?? 0 + losingTeam?.GetTaurusPlayers() ?? 0, true)
                .AddField("**Admins**", $"{StaticResources.Graphics[winning]} {winningTeam?.GetPlayersInTeam(mods)} - {losingTeam?.GetPlayersInTeam(mods)} {StaticResources.Graphics[losing]}", true)
